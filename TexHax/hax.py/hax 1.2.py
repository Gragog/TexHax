import os, sys, struct, re, subprocess, time
...
if len(sys.argv) == 2: #Assume BFRES
    ...
    proc1 = subprocess.Popen(['texconv2.exe', '-i', sys.argv[1], '-f', formats[0x41a], '-minmip', sys.argv[3], '-o', gtx1])
    proc1.wait()
    # i = 0
    # for x in xrange(1000000): i += 1 #Delay loop because texconv2 is slow
    # print("waiting for " + sys.argv[5] + " seconds...\n")
    ...
    proc2 = subprocess.Popen(['texconv2.exe', '-i', gtx1, '-f', formats[uint32(bfres, fpos+0x18)], '-o', gtx2])
    proc2.wait()
    time.sleep(int(sys.argv[5]))
    # i = 0
    # for x in xrange(1000000): i += 1 #Delay loop because texconv2 is slow
